...
def exportExcel():
    ...
    with open("students_" + dateFinal + ".csv", 'a', newline='') as f:
        ...
        w.writerow(['id', 'Profile Image', 'Student ID', 'First Name', 'Last Name', 'Phone #', 'Email', 'Address',
                    'Date Update'])
...
def renderTreeVIew(data):
    ...
    treeview = ttk.Treeview(treeFrame, show="headings", style="mystyle.Treeview", yscrollcommand=treeScroll.set,
                            columns=cols)
...
def renderAddWindow():
    def addStudent():
        ...
        if (studid == "" or studid == " ") or (fname == "" or fname == " ") or (lname == "" or lname == " ") or (
                phone == "" or phone == " ") or (email == "" or email == " ") or (address == "" or address == " "):
            ...
            try:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    f"INSERT INTO students (profile_image,stud_id,fname,lname,phone,email,address,date_update) VALUES ('{imageProfileFilename}','{studid}','{fname}','{lname}','{phone}','{email}','{address}','{getCurrentDate()}') ")
                conn.commit()
                conn.close()
            except:
    ...
    selectImageBtn = Button(master=addWindow, image=button_image_2, borderwidth=0, highlightthickness=0,
                            command=selectPic, relief="flat")
    ...
    addSubmitBtn = Button(master=addWindow, image=button_image_3, borderwidth=0, highlightthickness=0,
                          command=addStudent, relief="flat")
    ...
    cancelBtn = Button(master=addWindow, image=button_image_4, borderwidth=0, highlightthickness=0,
                       command=lambda: closeWindow(addWindow), relief="flat")
...
def renderEditWindow(selectedStudData):
        ...
        if (studid == "" or studid == " ") or (fname == "" or fname == " ") or (lname == "" or lname == " ") or (
            ...
            try:
                try:
                    ...
                    cursor = conn.cursor()
                    cursor.execute(
                        f"UPDATE students SET profile_image='{imageProfileFilename}',stud_id='{studid}',fname='{fname}',lname='{lname}',phone='{phone}',email='{email}',address='{address}',date_update='{getCurrentDate()}' WHERE stud_id='{selectedStudid}' ")
                    conn.commit()
                    conn.close()
                except:
                    conn = connection()
                    cursor = conn.cursor()
                    cursor.execute(
                        f"UPDATE students SET stud_id='{studid}',fname='{fname}',lname='{lname}',phone='{phone}',email='{email}',address='{address}',date_update='{getCurrentDate()}' WHERE stud_id='{selectedStudid}' ")
                    conn.commit()
                    conn.close()
                if os.path.exists("./assets/uploaded/temp.png"):
                    os.remove("./assets/uploaded/temp.png")
    ...
    editCanvas.create_text(49.0, 10.0, anchor="nw", text="Edit Student", fill="#FFFFFF",
    ...
    selectImageBtn = Button(master=editWindow, image=button_image_2, borderwidth=0, highlightthickness=0,
                            command=selectPic, relief="flat")
    selectImageBtn.place(x=196.0, y=215.0, width=96.0, height=25.0)

    button_image_3 = PhotoImage(master=editWindow, file=editWindowAssets("button_3.png"))
    submitBtn = Button(master=editWindow, image=button_image_3, borderwidth=0, highlightthickness=0,
                       command=editStudent, relief="flat")
    ...
    cancelBtn = Button(master=editWindow, image=button_image_4, borderwidth=0, highlightthickness=0,
                       command=lambda: closeWindow(editWindow), relief="flat")
    cancelBtn.place(x=137.0, y=402.0, width=96.0, height=25.0)
...
def renderViewWindow(selectedStudData):
    ...
    viewCanvas.create_text(49.0, 10.0, anchor="nw", text="View Student", fill="#FFFFFF",
                           font=("Inter SemiBold", 24 * -1))
...
mainCanvas.create_text(73.0, 15.0, anchor="nw", text="Student Profile Management System", fill="#FFFFFF",
...
style = ttk.Style()
style.configure("mystyle.Treeview", highlightthickness=0, bd=0, font=('Inter SemiBold', 12),
